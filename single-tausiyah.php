		...
		<?php while (have_posts()): the_post(); ?>
			...
			<div <?php post_class('single clear'); ?> id="post_<?php the_ID(); ?>">
				...
				<div class="post-content">
					...
					<strong>Kategori</strong> : <?php $terms = get_the_terms( $post->ID , 'kat-tausiyah' );
								if ( $terms != null ){
				...
				<div class="post-footer"><?php $terms = get_the_terms( $post->ID , 'topik' );
								if ( $terms != null ){
									foreach( $terms as $term ) {
			...
			<div class="clear" style="background: #72aa85; color: #fff ! important; padding: 10px; margin: 50px 0 20px;"><?php get_template_part('searchtausiyah'); ?></div>
		
		<?php endwhile; ?>
	<?php endif; ?>
	
	
	<?php query_posts('post_type=tausiyah&showposts=6'); ?>
	        <div class="single clear">
		    	<div class="post-meta">
			 		<h1>Tausiyah Lainnya</h1>
		    	</div>
			</div>
	<?php if (have_posts()): ?>
		<?php while (have_posts()): the_post(); ?>
